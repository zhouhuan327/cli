...
- 移动 Wiki 节点或将 Drive 文档迁入 Wiki：`wiki +move`
...
2. Create target folders / nodes shallow to deep.
3. Save returned tokens immediately.
4. Apply parent-child source move ordering.
5. Execute only confirmed scope.
6. Record success/failure per `PlanItem`.
7. Apply `Progress Reporting`.
